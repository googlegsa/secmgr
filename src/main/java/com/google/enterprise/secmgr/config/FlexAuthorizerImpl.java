...
public final class FlexAuthorizerImpl implements FlexAuthorizer {
  ...
  private void checkPriorityOrder(int order, int limit) {
    Preconditions.checkArgument(order >= 0 && order <= limit,
        "Priority order not in valid range: %s", order);
  }
  ...
  public synchronized boolean equals(Object object) {
    ...
    return Objects.equals(getAllRulesTable(), other.getAllRulesTable())
        && Objects.equals(getAllRoutingTable(), other.getAllRoutingTable());
  }

  @Override
  public synchronized int hashCode() {
    return Objects.hash(getAllRulesTable(), getAllRoutingTable());
  }
  ...
  public static FlexAuthorizer makeDefault(String authzServiceUrl, boolean samlUseBatchedRequests) {
    ...
    int index = 0;
    makeDefaultRule(flexAuthorizer, index++, FlexAuthzRule.ROOT_URL_PATTERN,
        AuthzMechanism.PER_URL_ACL,
    ...
    makeDefaultRule(flexAuthorizer, index++, FlexAuthzRule.HTTP_URL_PATTERN,
        AuthzMechanism.HEADREQUEST,
        FlexAuthzRule.LEGACY_AUTHN_ID);
    return flexAuthorizer;
  }

  static void makeDefaultRule(FlexAuthorizer flexAuthorizer, int index, String urlPattern,
  ...
  public static void makeDefaultRule(FlexAuthorizer flexAuthorizer, int index, String urlPattern,
      AuthzMechanism authzMechType, String authnId) {
    FlexAuthzRule rule
