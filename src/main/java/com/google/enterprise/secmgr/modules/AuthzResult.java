...
public final class AuthzResult implements Map<String, AuthzStatus> {
  ...
  /**
   * Get an {@link AuthzResult} builder that has a minimum set of resourceUrls.
   * This builder generates an AuthzResult instance that's guaranteed to have a
   * status for every resourceUrl in {@code resourceUrls}; those that aren't
   * explicitly set in the builder will map to {@link
   * AuthzStatus#INDETERMINATE}.
   *
   * @param resourceUrls The minimum set of resourceUrls to use for the returned
   *     builder.
   * @return An {@link AuthzResult} builder.
   */
  public static Builder builder(Iterable<String> resourceUrls) {
    Preconditions.checkNotNull(resourceUrls);
    return new Builder(resourceUrls);
  }
  ...
  /**
   * Get an {@link AuthzResult} builder that has a no minimum set of resourceUrls;
   * this is equivalent to passing an empty set of resourceUrls as an argument.
   *
   * @return An {@link AuthzResult} builder.
   */
  public static Builder builder() {
    ImmutableList<String> resourceUrls = ImmutableList.of();
    return new Builder(resourceUrls);
  }
   ...
   */
  public boolean wasDeterminedByCache(String resourceUrl) {
    return cacheResults.contains(resourceUrl);
  }
  ...
  public static final class Builder {
    ...
    private Builder(Iterable<String> resourceUrls) {
      map = Maps.newHashMap();
      for (String resourceUrl : resourceUrls) {
        map.put(resourceUrl, AuthzStatus.INDETERMINATE);
      }
     ...
     * @return The builder, for convenience.
     */
    public Builder putStatusFromCache(String resourceUrl, AuthzStatus status) {
      Preconditions.checkNotNull(resourceUrl);
      Preconditions.checkNotNull(status);
      map.put(resourceUrl, status);
      if (AuthzStatus.INDETERMINATE != status) {
        determinedByCache.add(resourceUrl);
      }
      return this;
    }
  ...
  /**
   * Make an {@link AuthzResult} in which all of the resourceUrls have
   * indeterminate status.
   *
   * @param resourceUrls The resourceUrls in the result.
   * @return An AuthzResult instance.
   */
  public static AuthzResult makeIndeterminate(Iterable<String> resourceUrls) {
    return builder(resourceUrls).build();
  }
  ...
  public String toString() {
    ...
    builder.append("{");
    for (String resourceUrl : Ordering.natural().sortedCopy(statusMap.keySet())) {
      if (!firstElt) {
        builder.append(',');
      } else {
        firstElt = false;
      ...
      builder.append("\", ");
      builder.append(statusMap.get(resourceUrl));
    }
    builder.append("\n}");
