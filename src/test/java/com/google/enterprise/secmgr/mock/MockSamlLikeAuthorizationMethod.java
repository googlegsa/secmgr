...
/**
 * This class mocks an external SAML authorization service. It can return
 * PERMIT, DENY and INDETERMINATE.
 */
public class MockSamlLikeAuthorizationMethod implements AuthorizationMethod {

  private final String key;
  private final String methodName;

  public MockSamlLikeAuthorizationMethod() {
    this(null);
  }

  public MockSamlLikeAuthorizationMethod(String key) {
    this.key = key;
  ...
  public AuthzResult authorize(Collection<Resource> resources, SessionSnapshot snapshot) {
    ...
    for (Resource resource : resources) {
      String match = (key != null) ? key : snapshot.getView().getUsername();
      builder.put(resource.getUrl(),
          resource.getUrl().contains(AuthorizationTestUtils.SI_PREFIX)
          ? AuthzStatus.INDETERMINATE
          : resource.getUrl().contains(match)
          ? AuthzStatus.PERMIT
          : AuthzStatus.DENY);
    }
