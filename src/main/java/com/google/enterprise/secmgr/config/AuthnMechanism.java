...
public abstract class AuthnMechanism {
  ...
  protected AuthnMechanism(String name) {
    // TODO: eliminate name == null once all callers supply real names.
  ...
  /**
   * Check a timeout argument.
   *
   * @param timeout The timeout argument to check.
   * @return A checked value.
   * @throws IllegalArgumentException if the argument is invalid.
   */
  protected static int checkTimeout(int timeout) {
    Preconditions.checkArgument(timeout >= NO_TIME_LIMIT);
    return timeout;
  }
  ...
  /**
   * Can the mechanism handle the request based on the headers?
   * Only supports one scheme for now.
   * @param request The original request
   * @return whether the mechanism can handle it or not
   */
  public boolean isApplicable(HttpServletRequest request) {
    String scheme = getScheme();
    if (scheme == null) {
      logger.fine("No scheme defined for the mechanism");
      return false;
    }

    if (request == null) {
      logger.fine("No request");
      return false;
    }

    String header = request.getHeader(HttpHeaders.AUTHORIZATION);
    if (header == null) {
      logger.fine("No authorization scheme");
      return false;
    }

    if (header.startsWith(scheme)) {
      logger.fine("Matching scheme");
      return true;
    }
    logger.fine("No matching scheme");
    return false;
  }
  ...
  /**
   * Gets the timeout for the request used by this mechanism.
   *
   * @return The timeout for this mechanism, in milliseconds.
   */
  ...
  public boolean hasTimeout() {
    return NO_TIME_LIMIT != getTimeout();
  }
    ...
    builder.registerTypeAdapter(AuthnMechanism.class,
        TypeAdapters.dispatch(
            ImmutableList.of(
                ...
                AuthnMechLdap.class,
                AuthnMechGroups.class,
                AuthnMechNtlm.class,
                AuthnMechPreauthenticated.class,
                AuthnMechSaml.class,
