...
public class SecurePasswordHasherTest extends TestCase {
  ...
  /**
   * Ensures there are no race conditions on getMac and getFingerprint.
   */
  public void testMacInputMultipleThreads() throws Exception {
    int numThreads = 10;

    List<Thread> hashThreads = new ArrayList<>();
    for (int i = 0; i < numThreads; i++) {
      Thread t = new Thread() {
        @Override public void run() {
          int numIterations = 100;
          for (int i = 0; i < numIterations; i++) {
            SecurePasswordHasher.getMac(kUsername, kPassword);
            SecurePasswordHasher.getFingerprint(kPassword);
          }
        }
      };
