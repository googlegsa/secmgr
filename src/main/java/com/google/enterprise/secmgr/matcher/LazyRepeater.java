 ...
 * input sequence a specified number of times.  This matcher matches lazily,
 * meaning that it matches the minimum number of times, and then increases the
...
final class LazyRepeater extends Matcher {
  @Nonnull private final Matcher matcher;
  @Nonnegative private final int min;
  @Nonnegative private final int max;

  private LazyRepeater(Matcher matcher, @Nonnegative int min, @Nonnegative int max) {
  ...
  /**
   * Gets a lazy repeated matcher by wrapping a given matcher.
   *
  ...
  static Matcher make(Matcher matcher, @Nonnegative int min, @Nonnegative int max) {
    return new LazyRepeater(matcher, min, max);
  }
