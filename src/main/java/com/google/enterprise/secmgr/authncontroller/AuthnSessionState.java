...
public final class AuthnSessionState implements Serializable {
  ...
  private static final class Evolver {
    ...
    Evolver(ImmutableList<CredentialGroup> credentialGroups,
      ...
      this.credentialsMap = MultimapBuilder.hashKeys().hashSetValues().build(credentialsMap);
    ...
    void addCredential(AuthnAuthority authority, Credential credential) {
      ...
      if (!credentialsMap.containsEntry(cgAuthority, credential)) {
        Iterator<Credential> iter = credentialsMap.get(cgAuthority).iterator();
        while (iter.hasNext()) {
          Credential c = iter.next();
          if (credential.getClass().isInstance(c)) {
            iter.remove();
            switch(credential.getTypeName()) {
              case PRINCIPAL:
              case PASSWORD:
                for (AuthnAuthority mechAuthority : getMechAuthorities(cgAuthority)) {
                  ...
                  if (verification != null && verification.getCredentials().contains(c)) {
                    verificationsMap.remove(mechAuthority);
                  }
                }
                break;
              // since some authn mechs can have only groups lookup enabled, 
              // old verifications should stay.
              case GROUPS:
                break;
              default:
              // only credential types which implement Credential can be added here.
                throw new IllegalArgumentException("Unknown credential type : " 
                                                    + credential.getTypeName());
            }
          }
        }
    ...
    void removeCredential(AuthnAuthority authority, Credential credential) {
      ...
      if (credentialsMap.containsEntry(cgAuthority, credential)) {
        for (AuthnAuthority mechAuthority : getMechAuthorities(cgAuthority)) {
          Verification verification = verificationsMap.get(mechAuthority);
          if (verification != null && verification.getCredentials().contains(credential)) {
            verificationsMap.remove(mechAuthority);
          }
        }
  ...
  public static final class Instruction implements Serializable {
    ...
    public boolean equals(Object object) {
      ...
      return Objects.equals(getOperation(), other.getOperation())
          && Objects.equals(getAuthority(), other.getAuthority())
          && Objects.equals(getOperand(), other.getOperand());
    }

    @Override
    public int hashCode() {
      return Objects.hash(getOperation(), getAuthority(), getOperand());
    }
